...
// Handwritten Digit Recognition using HOG and SVM
// source: downloaded from  https://www.learnopencv.com/handwritten-digits-classification-an-opencv-c-python-tutorial/
...
// function for deskewing the digits
Mat deskew(Mat& img) {
	...
	// calculate affine transform to correct skew
...
void SVMtrain(Mat &trainMat, vector<int> &trainLabels, Mat &testResponse, Mat &testMat) {
	...
	svm->setC(12.5);
	svm->setKernel(SVM::RBF); // set kernel to Radial Basis Function
	svm->setType(SVM::C_SVC);

	// train SVM on training data
	Ptr<TrainData> td = TrainData::create(trainMat, ROW_SAMPLE, trainLabels);
	svm->train(td);
	//svm->trainAuto(td); // this option chooses C and Gamma for you, but takes 5x longer than svm->train
